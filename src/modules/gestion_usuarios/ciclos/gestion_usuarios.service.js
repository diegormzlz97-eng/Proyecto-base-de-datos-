...
export class gestion_usuariosService {
    ...
    static async actualizarNombrePorId ({nombre, id}){
        ...
        if(!result){ throw new gestion_usuariosError("El usuario no existe", 401) }
        
        //Actualizar el nombre del usuario
        const actualizacion = await UsuariosModel.actualizarNombrePorId({ nombre, id })
        if(!actualizacion.status){ throw new gestion_usuariosError(actualizacion.message, 401) }
    ...
    static async eliminarPorId ({id}){
        ...
        if(!eliminar.status){ throw new gestion_usuariosError(eliminar.message, 401) }
